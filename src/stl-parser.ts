...
function _parseASCII(data: string) {
...
function _matchDataViewAt(query: number[], reader: DataView, offset: number) {
...
function _isBinary(data: ArrayBuffer) {
	const reader = new DataView(data);
	...
	for (let offset = 0; offset < 5; offset++) {
		...
		if (_matchDataViewAt(solid, reader, offset)) return false;
...
function _ensureBinary(buffer: ArrayBuffer | string) {
...
function _ensureString(buffer: ArrayBuffer | string) {
	if (typeof buffer !== 'string') {
